...
class asistencia(models.Model):
     _name = 'colegio.asistencia'
     _description = 'Modelo o tabla asistencia'
    
    
     curso_materia_id = fields.Many2one('colegio.curso_materia', string="Curso Materia", required=True)
     estudiante_id = fields.Many2one('colegio.estudiante', string="Estudiante")
     gestion_id = fields.Many2one('colegio.gestion', string="Gestion", compute='_compute_gestion_id', store=True)
     fecha = fields.Datetime(string="Fecha", default=fields.Date.context_today, required=True)
     asistencia_line_ids = fields.One2many('colegio.asistencialine', 'asistencia_id', string="Líneas de Asistencia")
     ...
     def load_estudiantes(self):
        for record in self:
            if record.curso_materia_id and record.gestion_id:
                estudiantes = self.env['colegio.inscripcion'].search([
     ...
     def _check_profesor_curso_materia(self):
        ...
        profesor = self.env['colegio.profesor'].search([('usuario_id', '=', usuario_id)], limit=1)
        for record in self:
...
class asistencialine(models.Model):
      _name = 'colegio.asistencialine'
      _description = 'Línea de Asistencia'

      asistencia_id = fields.Many2one('colegio.asistencia', string="Asistencia", required=True, ondelete='cascade')
...
class asistenciawizard(models.Model):
    _name = 'colegio.asistenciawizard'
    _description = 'Wizard para registrar asistencia'

    curso_materia_id = fields.Many2one('colegio.curso_materia', string="Curso Materia", required=True)
    asistencia_line_ids = fields.One2many('colegio.asistencia_wizard_line', 'wizard_id', string="Asistencia")
    ...
    #         self.env['colegio.asistencia'].create({
...
class asistenciawizardline(models.TransientModel):
    _name = 'colegio.asistencia_wizard_line'
    _description = 'Línea del Wizard de Asistencia'

    wizard_id = fields.Many2one('colegio.asistencia_wizard', string="Wizard")
    estudiante_id = fields.Many2one('colegio.estudiante', string="Estudiante", required=True)
    curso_materia_id = fields.Many2one('colegio.curso_materia', string="Curso Materia", required=True)
