...
class curso_materia(models.Model):
    _name = 'colegio.curso_materia'
    _description = 'Modelo o tabla curso materia'
    curso_id = fields.Many2one('colegio.curso', string="Curso", ondelete='cascade', required=True)
    materia_id = fields.Many2one('colegio.materia', string="Materia", ondelete='cascade', required=True)
    profesor_id = fields.Many2one('colegio.profesor', string="Profesor", ondelete='cascade', required=True)
    horario_id = fields.Many2one('colegio.horario', string="Horario", ondelete='cascade', required=True)
    nota_ids = fields.One2many('colegio.nota', 'curso_materia_id', string="Cursos_Materias_nota")
    
    
    subnota_ids = fields.One2many('colegio.subnota', 'curso_materia_id', string="Cursos_Materias_subnota")
    gestion_id = fields.Many2one('colegio.gestion', string="Gestion", ondelete='cascade', required=True) 
    
    @api.constrains('horario_id')
    def _check_horario_conflict(self):
        for record in self:
            existing_records = self.env['colegio.curso_materia'].search([
