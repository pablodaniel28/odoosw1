...
class inscripcion(models.Model):
     ...
     estudiante = fields.Many2one('colegio.estudiante', string="Estudiante", ondelete='cascade', required=True)
     estudiante_nombre = fields.Char(related='estudiante.nombre', string='Estudiante')
    
     curso = fields.Many2one('colegio.curso', string="Curso", ondelete='cascade', required=True)
     ...
     mensualidad_ids = fields.One2many('colegio.mensualidad', 'inscripcion_id', string="Inscripciones")
     subnota_ids = fields.One2many('colegio.subnota', 'subinscripcion_id', string="subnotas") 
     nota_ids = fields.One2many('colegio.nota', 'noinscripcion_id', string="Estudiantes_notas")
