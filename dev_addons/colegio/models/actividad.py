...
class Actividad(models.Model):
    ...
    actividad_usuario_ids = fields.One2many('colegio.actividad_usuario', 'actividad_id', string="Actividad")
    curso_id = fields.Many2one('colegio.curso', string="Curso", required=True)
    agenda_id = fields.Many2one('colegio.agenda', string="Agenda", ondelete='cascade')
    ...
    def create_actividad_for_course_users(self):
        ...
        actividad_usuario_model = self.env['colegio.actividad_usuario']
        for estudiante in estudiante_ids:
            usuario_id = self.env['colegio.estudiante'].browse(estudiante).usuario_id.id
