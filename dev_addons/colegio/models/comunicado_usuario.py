...
class comunicado_usuario(models.Model):
    _name = 'colegio.comunicado_usuario'
    _description = 'Modelo o tabla comunicado_usuario'

    visto = fields.Boolean(string='Visto', default=False, required=True)
    comunicado_id = fields.Many2one('colegio.comunicado', string="Comunicado", ondelete='cascade', required=True)
    ...
    def _get_domain_usuarios(self):
        tutores = self.env['colegio.tutor'].search([])
    ...
    def _compute_tutor_usuario_info(self):
        for record in self:
            if record.usuario_recibe_id:
                tutor = self.env['colegio.tutor'].search([('usuario_id', '=', record.usuario_recibe_id.id)], limit=1)
                if tutor:
    ...
    #usuario_recibe_id = fields.Many2one('colegio.tutor', string='UsuarioFinal',required=True)
