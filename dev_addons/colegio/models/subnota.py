...
class subnota(models.Model):
    ...
    curso_materia_id = fields.Many2one('colegio.curso_materia', string="Curso_Materia", ondelete='cascade', required=True)
    ...
    estudiante_id = fields.Many2one('colegio.estudiante', string="Estudiante", ondelete='cascade')
    subinscripcion_id = fields.Many2one('colegio.inscripcion', string="Inscripcion", ondelete='cascade',required=True)
    ...
    def _compute_notafinal(self):
        for record in self:
            subnotas = self.env['colegio.subnota'].search([
    ...
    #     for record in self:
    #         existing_subnota = self.env['colegio.subnota'].search([
    #             ('curso_materia_id', '=', record.curso_materia_id.id),
    #             ('estudiante_id', '=', record.estudiante_id.id),
